...
import { supabase, getUser, sendMagicLink } from '../lib/supabase'
...
// ─── ContractorDigestToggle ───────────────────────────────────────────────────

function ContractorDigestToggle({ userId }) {
...
export default function ContractorMode() {
  ...
  async function loadAll() {
    ...
    try {
      ...
      // No Supabase session — show magic link prompt immediately
      // They have localStorage tier access but haven't authenticated yet
      if (!currentUser) {
        setNeedsAuth(true)
        setLoading(false)
        return
      ...
      {needsAuth && (
        <div className="bg-brand-50 border border-brand-100 rounded-xl px-5 py-4 mb-5">
          ...
          <div className="text-xs text-brand-700 mb-3 leading-relaxed">
            Your Contractor access is confirmed. Enter your email below and we'll send a magic link to log you in — your jobs and profile will load automatically.
          </div>
          {magicSent ? (
            <div className="text-sm text-green-700 font-medium">✓ Check your email for the login link — then come back here.</div>
          ) : (
                ...
                <div key={job.id} className="bg-white border border-gray-100 rounded-xl p-4 hover:border-gray-200 transition-colors">
                  <div className="flex items-center justify-between gap-3 mb-2">
                    ...
                    <div className="flex items-center gap-1 flex-shrink-0">
                      ...
                      {['durham', 'raleigh'].includes(job.jurisdiction) && (
                        <Link
                          to={`/apply?a=${encodeURIComponent(job.address || '')}&p=${job.project_type || 'sfh'}&s=0&j=${job.jurisdiction}`}
      ...
      {tab === t('profile_title') && (
        <div>
          ...
          <div className="mt-8 pt-6 border-t border-gray-100 max-w-lg">
            ...
            <p className="text-xs text-gray-400 mb-4">Sent every Monday morning with a summary of your active jobs and any items needing attention.</p>
            <ContractorDigestToggle userId={user?.id} />
          </div>
        </div>
      )}
